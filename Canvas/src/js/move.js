class Move {
    static makeDataMoveleft(player, celuleObstacle, move) {
        ...
        for (let index = 0; index < move; index++) {
            ...
            celuleObstacle.forEach((obstacle) => {
                if (
                    obstacle.width == moveLeft[index].width &&
                    obstacle.height == moveLeft[index].height
                ) {
    ...
    static makeDataMoveTop(player, celuleObstacle, move) {
        ...
        celuleObstacle.forEach((obstacle) => {
            if (
                obstacle.width == player.width &&
                obstacle.height + 50 == player.height
            ) {
                moveLeftAfterObstacle = 0;
            } else if (
                obstacle.width == player.width &&
                obstacle.height + 50 == player.height - 50
            ) {
                moveLeftAfterObstacle = 50;
            } else if (
                obstacle.width == player.width &&
                obstacle.height + 50 == player.height - 100
            ) {
                moveLeftAfterObstacle = 100;
            }
        });
        player.move.top = moveLeftAfterObstacle;
        /*
    let moveLeft = [];
    ...
    for (let y = 0; y < saveObstacle.length; y++) {
      if (moveLeft[0].height == saveObstacle[0]) {
        ...
        if (
          moveLeft[1].height == saveObstacle[1] ||
          moveLeft[1].height == saveObstacle[0]
        ) {
    ...
    static makeDataMoveRight(player, celuleObstacle, move) {
        ...
        for (let y = 0; y < saveObstacle.length; y++) {
            if (moveLeft[0].width == saveObstacle[0]) {
                ...
                if (
                    moveLeft[1].width == saveObstacle[1] ||
                    moveLeft[1].width == saveObstacle[0]
                ) {
